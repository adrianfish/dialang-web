...
$.get('/content/itemreview/' + dialang.session.al + '.html', function (data) {
    ...
    var rows = [];
    for (var subskillKey in dialang.session.subskills) {
    ...
    $.get('/templates/itemreviewtable.mustache', function (template) {
        ...
        $('.itemreview-button').click(function (e) {
            ...
            dialang.session.items.forEach(function (item) {
            ...
            if(clickedItem !== null) {
                dialang.session.reviewBasket = dialang.session.baskets[dialang.session.itemToBasketMap[clickedItem.id]];
                ...
                console.error('The clicked item was not in dialang.session.items.');
            }
            return false;
        });
    },'text');
