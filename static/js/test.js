...
if (!dialang.session.reviewMode) {
    ...
    $.get('/content/baskets/' + dialang.session.al + "/" + dialang.session.currentBasketId + '.html', function (data) {
        ...
        if (dialang.session.currentBasketType === 'gaptext'
                || dialang.session.currentBasketType === 'shortanswer') {
        ...
        if (numItemsInThisBasket === 1) {
            $('#progressbar-label').html(dialang.session.items.length + 1);
        } else {
            $('#progressbar-label').html((dialang.session.items.length + 1) + "-" + (dialang.session.items.length + numItemsInThisBasket));
        }
        ...
        if ('mcq' === dialang.session.currentBasketType) {
            $.getScript('/js/mcqresponse.js');
        } else if ('gapdrop' === dialang.session.currentBasketType) {
            $.getScript('/js/gapdropresponse.js');
        } else if ('gaptext' === dialang.session.currentBasketType) {
            $.getScript('/js/gaptextresponse.js');
        } else if ('shortanswer' === dialang.session.currentBasketType) {
            $.getScript('/js/shortanswerresponse.js');
        } else if ('tabbedpane' === dialang.session.currentBasketType) {
